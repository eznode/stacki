...
class Bootable(stack.bootable.Bootable):
	...
	def installBootfiles(self, dstPath):
		...
		with open(os.path.join(dstISOLinuxPath, 'isolinux.cfg'), 'w') as fout:
			...
			fout.write('\tappend initrd=initrd.img autoyast=file:///tmp/profile/autoinst.xml ramdisk_size=300000 biosdevname=0 Server=127.0.0.1 install=cd:/isolinux/image\n\n')
